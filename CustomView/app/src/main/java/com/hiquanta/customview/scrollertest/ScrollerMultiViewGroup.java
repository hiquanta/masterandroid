 ...
 */
public class ScrollerMultiViewGroup extends ViewGroup {

    private Context mContext;

    private static String TAG = "ScrollerMultiViewGroup";

    private int curScreen = 0;  //当前屏

    private Scroller mScroller = null;

    public ScrollerMultiViewGroup(Context context) {
        super(context);
        mContext = context;
        init();
    }

    public ScrollerMultiViewGroup(Context context, AttributeSet attrs) {
        super(context, attrs);
        mContext = context;
        init();
    }
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        for (int i = 0; i < childCount; i++) {
            ...
            // 设置每个子视图的大小 ， 即全屏
            child.measure(getWidth(), ScrollerMultiScreenActivity.scrrenHeight);
        }
    }
        ...
        for (int i = 0; i < childCount; i++) {
            ...
            if (child.getVisibility() != View.GONE)
                child.layout(startLeft, startTop,
                        startLeft + getWidth(),
                        startTop + ScrollerMultiScreenActivity.scrrenHeight);
