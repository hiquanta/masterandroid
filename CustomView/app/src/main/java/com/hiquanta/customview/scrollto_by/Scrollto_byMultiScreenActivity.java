...
//带有可以切换屏的Activity
public class Scrollto_byMultiScreenActivity extends Activity implements OnClickListener {
		...
		switch (v.getId()) {
			case R.id.bt_scrollLeft:
				if(curscreen > 0) {  //防止屏幕越界
					curscreen -- ;
					Toast.makeText(Scrollto_byMultiScreenActivity.this, "第" +(curscreen+1) + "屏", 300).show();
				}
				else
					Toast.makeText(Scrollto_byMultiScreenActivity.this, "当前已是第一屏",300).show();
			...
			case R.id.bt_scrollRight:
				if (curscreen < 2 ){ //防止屏幕越界
					...
					Toast.makeText(Scrollto_byMultiScreenActivity.this, "当前已是最后一屏",300).show();
				mulTiViewGroup.scrollTo(curscreen * screenWidth, 0);
				break;
		}
	}
